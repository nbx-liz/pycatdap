...
- pysubgroup interop measure `pycatdap.measures.AICMeasure` を追加 (#31, H-0018)。
  ...
  pysubgroup は optional 依存 (`pip install 'pycatdap[subgroup]'`)。`pycatdap.measures` の
  import は pysubgroup を必須としない (lazy export)。ΔAIC に健全な上界がないため
  `optimistic_estimate` は提供せず、Apriori / DFS は対象外 (BeamSearch / SimpleDFS 専用)。
  ガイド: `docs/interop/pysubgroup.md`。
